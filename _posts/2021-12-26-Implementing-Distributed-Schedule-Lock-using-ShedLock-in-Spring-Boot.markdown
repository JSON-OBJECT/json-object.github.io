...
dependencies {
    ...
    // 데이터 저장소로 DynamoDB를 사용할 경우 추가
    implementation("net.javacrumbs.shedlock:shedlock-provider-dynamodb2:5.8.0")
    implementation("software.amazon.awssdk:dynamodb-enhanced:2.20.145")
    
    // 다른 옵션으로 데이터 저장소로 RDBMS를 사용할 경우 추가
    implementation("net.javacrumbs.shedlock:shedlock-provider-jdbc-template:5.8.0")
}
```

### application.yml
  * 프로젝트의 **/src/main/resources/application.yml**에 아래 내용을 추가한다.
...
class ScheduleService {
    ...
    // 매일 KST 04:30:00에 단 1개의 노드에서만 스케쥴 작업을 실행
